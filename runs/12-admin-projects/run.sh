...
run_sample "SR-210" "admin-project-list" \
  "raps admin project list -a $ACCOUNT_ID || true" \
...
run_sample "SR-211" "admin-project-list-filtered" \
  "raps admin project list -a $ACCOUNT_ID -f \"name:*Tower*\" --status active --platform acc --limit 10 || true" \
...
run_sample "SR-212" "admin-project-create" \
  "raps admin project create -a $ACCOUNT_ID --name \"Tower Phase 3\" -t \"Bridge\" --classification \"Sample\" --start-date \"2026-03-01\" --end-date \"2027-12-31\" --timezone \"America/New_York\" || true" \
...
run_sample "SR-213" "admin-project-update" \
  "raps admin project update -a $ACCOUNT_ID -p $PROJECT_ID --name \"Tower Phase 3 - Revised\" --status active || true" \
...
run_sample "SR-214" "admin-project-archive" \
  "raps admin project archive -a $ACCOUNT_ID -p $PROJECT_ID || true" \
