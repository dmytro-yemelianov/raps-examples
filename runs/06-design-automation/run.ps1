...
Invoke-Sample -Id "SR-112" -Slug "da-appbundle-create" `
  -Command "raps da appbundle-create -i `"CountWalls`" -e `"Autodesk.Revit+2025`" -d `"Count walls plugin`"" `
...
Invoke-Sample -Id "SR-113" -Slug "da-appbundle-delete" `
  -Command "raps da appbundle-delete `"CountWalls`"" `
...
Invoke-Sample -Id "SR-115" -Slug "da-activity-create" `
  -Command "raps da activity-create --id `"CountWallsActivity`" --engine `"Autodesk.Revit+2025`" --appbundle `"CountWalls`" --command `"...`"" `
...
Invoke-Sample -Id "SR-116" -Slug "da-activity-delete" `
  -Command "raps da activity-delete `"CountWallsActivity`"" `
...
Invoke-Sample -Id "SR-117" -Slug "da-run" `
  -Command "raps da run `"CountWallsActivity`" -i input=$env:SIGNED_URL -o output=$env:OUTPUT_URL -w" `
...
Invoke-Sample -Id "SR-119" -Slug "da-status" `
  -Command "raps da status $env:WORKITEM_ID" `
...
Invoke-LifecycleStep -StepNum 2 -Command "raps da appbundle-create -i `"ExtractData`" -e `"Autodesk.Revit+2025`" -d `"Extract data plugin`""
Invoke-LifecycleStep -StepNum 3 -Command "raps da appbundles"
Invoke-LifecycleStep -StepNum 4 -Command "raps da activity-create --id `"ExtractAct`" --engine `"Autodesk.Revit+2025`" --appbundle `"ExtractData`" --command `"...`""
Invoke-LifecycleStep -StepNum 5 -Command "raps da activities"
Invoke-LifecycleStep -StepNum 6 -Command "raps da activity-delete `"ExtractAct`""
Invoke-LifecycleStep -StepNum 7 -Command "raps da appbundle-delete `"ExtractData`""
End-Lifecycle
