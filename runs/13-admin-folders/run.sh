...
run_sample "SR-220" "admin-folder-rights-dryrun" \
  "raps admin folder set-permissions $TEST_USER -a $ACCT -l view-download-upload --folder \"Plans\" -f \"name:*Tower*\" --dry-run || true" \
...
run_sample "SR-221" "admin-folder-rights-execute" \
  "raps admin folder set-permissions $TEST_USER -a $ACCT -l view-download-upload --folder \"Plans\" -f \"name:*Tower*\" -y || true" \
...
run_sample "SR-222" "admin-folder-rights-from-file" \
  "raps admin folder set-permissions $TEST_USER -a $ACCT -l folder-control --project-ids ./projects.txt -y || true" \
...
lifecycle_step 1 "raps admin folder set-permissions $TEST_USER_FOLDER -a $ACCT -l view-download-upload-edit --folder \"Plans\" -f \"name:*Active*\" --dry-run"
lifecycle_step 2 "raps admin folder set-permissions $TEST_USER_FOLDER -a $ACCT -l view-download-upload-edit --folder \"Plans\" -f \"name:*Active*\" -y"
lifecycle_step 3 "raps admin operation list --limit 1"
lifecycle_step 4 "raps admin operation status $OP_ID"
lifecycle_step 5 "raps admin folder set-permissions $TEST_USER_FOLDER -a $ACCT -l view-only --folder \"Plans\" -f \"name:*Active*\" -y"
