...
run_sample "SR-253" "template-update" \
  "raps template update \$TEMPLATE_ID --account \$ACCOUNT_ID --name \"Standard Building Template v2\"" \
  "Expected: Updates template" \
  "Review: Exit 0; name changed"

# SR-254: Archive a template
run_sample "SR-254" "template-archive" \
  "raps template archive \$TEMPLATE_ID --account \$ACCOUNT_ID" \
...
lifecycle_step 1 "raps template create --account \$ACCT --name \"Healthcare Template\""
lifecycle_step 2 "raps template list --account \$ACCT"
lifecycle_step 3 "raps template info \$TPL_ID --account \$ACCT"
lifecycle_step 4 "raps template update \$TPL_ID --account \$ACCT --name \"Healthcare Template 2026\""
lifecycle_step 5 "raps template archive \$TPL_ID --account \$ACCT"
lifecycle_step 6 "raps template list --account \$ACCT"
lifecycle_end
