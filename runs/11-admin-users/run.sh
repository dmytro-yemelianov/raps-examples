...
run_sample "SR-193" "admin-user-add-bulk-dryrun" \
  "raps admin user add user@company.com -a $ACCOUNT_ID -r \"project_admin\" -f \"name:*Tower*\" --dry-run || true" \
...
run_sample "SR-194" "admin-user-add-bulk-execute" \
  "raps admin user add user@company.com -a $ACCOUNT_ID -r \"project_admin\" -f \"name:*Tower*\" -y || true" \
...
run_sample "SR-195" "admin-user-add-from-file" \
  "raps admin user add user@company.com -a $ACCOUNT_ID -r \"viewer\" --project-ids ./project-ids.txt -y || true" \
...
run_sample "SR-196" "admin-user-remove-bulk-dryrun" \
  "raps admin user remove user@company.com -a $ACCOUNT_ID -f \"name:*Old Project*\" --dry-run || true" \
...
run_sample "SR-197" "admin-user-update-bulk-dryrun" \
  "raps admin user update user@company.com -a $ACCOUNT_ID -r \"viewer\" --from-role \"project_admin\" -f \"name:*Archive*\" --dry-run || true" \
...
run_sample "SR-198" "admin-user-update-from-csv" \
  "raps admin user update user@company.com -a $ACCOUNT_ID --from-csv ./role-changes.csv -y || true" \
...
run_sample "SR-199" "admin-user-add-single" \
  "raps admin user add-to-project -p $PROJECT_ID -e \"new.user@company.com\" -r $ROLE_ID || true" \
...
lifecycle_step 1 "raps admin user list -a $ACCT --search \"newuser@company.com\"" || true
lifecycle_step 2 "raps admin project list -a $ACCT --status active -f \"name:*Building*\"" || true
lifecycle_step 3 "raps admin user add newuser@company.com -a $ACCT -r \"project_admin\" -f \"name:*Building*\" --dry-run" || true
lifecycle_step 4 "raps admin user add newuser@company.com -a $ACCT -r \"project_admin\" -f \"name:*Building*\" -y" || true
lifecycle_step 5 "raps admin user list -a $ACCT --search \"newuser@company.com\"" || true
lifecycle_step 6 "raps admin folder rights newuser@company.com -a $ACCT -l view-download-upload --folder \"Plans\" -f \"name:*Building*\" --dry-run" || true
lifecycle_step 7 "raps admin folder rights newuser@company.com -a $ACCT -l view-download-upload --folder \"Plans\" -f \"name:*Building*\" -y" || true
lifecycle_end

# SR-204: Remove departing employee
lifecycle_start "SR-204" "employee-offboarding" "Remove departing employee"
lifecycle_step 1 "raps admin user list -a $ACCT --search \"departing@company.com\"" || true
lifecycle_step 2 "raps admin user remove departing@company.com -a $ACCT --dry-run" || true
lifecycle_step 3 "raps admin user remove departing@company.com -a $ACCT -y" || true
lifecycle_step 4 "raps admin user list -a $ACCT --search \"departing@company.com\"" || true
lifecycle_end

# SR-205: Downgrade stale admins to viewers
lifecycle_start "SR-205" "role-migration" "Downgrade stale admins to viewers"
lifecycle_step 1 "raps admin project list -a $ACCT --status active -f \"name:*2024*\"" || true
lifecycle_step 2 "raps admin user list -a $ACCT --role \"project_admin\"" || true
lifecycle_step 3 "raps admin user update admin1@co.com -a $ACCT -r \"viewer\" --from-role \"project_admin\" -f \"name:*2024*\" --dry-run" || true
lifecycle_step 4 "raps admin user update admin1@co.com -a $ACCT -r \"viewer\" --from-role \"project_admin\" -f \"name:*2024*\" -y" || true
lifecycle_step 5 "raps admin user list -a $ACCT -p $OLD_PROJECT --role \"project_admin\"" || true
lifecycle_end

# SR-206: Onboard 50 users from CSV
lifecycle_start "SR-206" "csv-batch-onboarding" "Onboard 50 users from CSV"
lifecycle_step 1 "raps admin user import -p $PROJECT_ID --from-csv ./bulk-users.csv" || true
lifecycle_step 2 "raps admin user list -a $ACCT -p $PROJECT_ID" || true
lifecycle_step 3 "raps admin user update user1@co.com -a $ACCT --from-csv ./role-updates.csv -y" || true
lifecycle_step 4 "raps admin user list -a $ACCT -p $PROJECT_ID --role \"project_admin\"" || true
lifecycle_end
